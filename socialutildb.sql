...
CREATE TABLE `chat`  (
  ...
  `from_` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL COMMENT '发送者',
  `to_` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL COMMENT '接收者',
  ...
  `date` varchar(15) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL COMMENT '时间',
  `illegal` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT NULL COMMENT '违法原因',
  PRIMARY KEY (`chatid`) USING BTREE,
  INDEX `chat_to`(`to_`) USING BTREE,
  INDEX `chat_state`(`state`) USING BTREE,
  INDEX `chat_from`(`from_`) USING BTREE,
  CONSTRAINT `chat_from` FOREIGN KEY (`from_`) REFERENCES `user` (`account`) ON DELETE NO ACTION ON UPDATE CASCADE,
  CONSTRAINT `chat_to` FOREIGN KEY (`to_`) REFERENCES `user` (`account`) ON DELETE NO ACTION ON UPDATE CASCADE
...
CREATE TABLE `own`  (
  ...
  `info` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT NULL COMMENT '个人圈信息',
  `picture` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci DEFAULT NULL COMMENT '个人圈图片',
  `tags` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT NULL COMMENT '标签',
  `fakename` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT NULL COMMENT '昵称',
  ...
  `friendid` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci DEFAULT NULL COMMENT '好友id',
  `singlesex` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT NULL COMMENT '个性签名',
  `redate` varchar(255) CHARACTER SET latin1 COLLATE latin1_bin DEFAULT NULL,
...
CREATE TABLE `school`  (
  ...
  `name` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci DEFAULT NULL COMMENT '姓名',
  PRIMARY KEY (`id`) USING BTREE,
...
CREATE TABLE `user`  (
  ...
  `state` int(1) DEFAULT NULL COMMENT '0表示正常   1表示警告  2表示封号',
  PRIMARY KEY (`id`) USING BTREE,
